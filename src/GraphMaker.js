...
function mapDispatchToProps(dispatch) {
  return {
    ...
    setLinkOptions: linkOptions =>
      dispatch({ type: "SET_LINK_OPTIONS", linkOptions }),
...
class GraphMaker extends React.Component {
  ...
  render() {
    ...
    if (groups && Object.keys(groups).length > 0) {
      const hullFromGroup = group =>
        d3Poly.polygonHull(
          group.nodeIDs.map(id => [nodes[id].x + 75, nodes[id].y + 20])
        );
      const hullPath = hull => {
        if (hull !== undefined && hull !== null) {
          return (
            hull
              .splice(1)
              .reduce((acc, d) => acc + "L" + d.join(), "M" + hull[0].join()) +
            "Z"
          );
        } else {
      ...
      hulls = _.map(groups, group => ({
        id: group.id,
        hullPath: hullPath(hullFromGroup(group)),
        color: group.color
      })); //
    }
    return (
...
const fireBasedComponent = firebaseConnect(["/graphs", "/graphNames"])(
  connected
);
