  ...
  render() {
    ...
    return (
      <ZoomDivContainer width={graphWidth} height={graphHeight}
        onClick={e => { e$Out(e$.clickBG.str, e) }}
        onContextMenu={e => { e.preventDefault(); e$Out(e$.rightClickBG.str, e) }}
        onMouseMove={e => { e$Out(e$.mouseMove.str, e) }}
        onMouseDown={e => {
          const { offsetX, offsetY, clientX, clientY, button } = e.nativeEvent
          const buttonFromNum = {0: 'left', 1: 'middle', 2:'right'}
          rxBus.next({
            type: buttonFromNum[button] + 'MouseDown',
            offsetX, offsetY, clientX, clientY, button
          })
        }}
